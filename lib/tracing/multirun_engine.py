...
from lib.tracing import network as network
from lib.tracing.utils import get_pool, tqdm_redirect, is_not_empty, no_std_context, get_stateless_sampling_func, ListDelegator
from lib.tracing.stats import StatsEvent
from lib.tracing.models import add_trans
...
class EngineNet(Engine):
    ...
    def __call__(self, inet, return_last_net=False):
        ...
        try:
            true_net = network.get_from_predef(nettype['0'][0], inet=inet, W_factor=nettype.get('Wi', 0), count_importance=tr_rate, **args_dict)
...
#                 print(e)
            ...
            if args.is_dynamic and current_time >= args.update_after * update_iter:
                ...
                try:
                    ...
                    edges_to_update = args.nettype[str(update_iter)]
                    if edges_to_update:
...
class EngineOne(Engine):
    ...
    def __call__(self, itr):
        ...
        if args.agent is not None:
            ...
            initial_known = int(args.control_initial_known if args.control_initial_known >= 1 else args.control_initial_known * max(inf, control_after_inf))
        ...
        # iterator needed for external control; initialized to first timepoint for control
        control_iter = 0
...
#             print(e)
            ...
            if any_inf and separate_traced:
                ...
                if agent and current_time >= control_after + control_iter and control_after_inf <= m['totalInfected']:
                    if is_record_agent:
                        agent.control(true_net, net_changed=net_changed)
                    else:
                        ...
                        initial_known_ids = np.where(true_net.node_infected)[0][:initial_known] if control_iter == 0 else ()
                        pos, traced = agent.control(true_net, control_iter, initial_known_ids, net_changed)
                        isolated = len(pos) + len(traced)
                        m['totalTraced'] += isolated
                        node_states = true_net.node_states
                        for nid in pos:
                            state = node_states[nid]
                            event = sim_true.get_event_with_config(node=nid, fr=state, to='T', time=current_time)
                            # remove nid from traced_time to stop it from influencing the tracing chances of
                            # its neighbors after self-isolation exit
                            sim_true.run_trace_event_for_infect_net(event, to_traced=True)
        ...
        if is_record_agent:
            ...
            if agent.lr and agent.rl_sampler and not agent.online:
                ...
                agent.update_parameters(last_reward=-sum(true_net.node_infected))
            if agent.gpu:
                agent.torch.cuda.empty_cache()
