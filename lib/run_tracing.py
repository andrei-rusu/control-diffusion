...
from lib.tracing.multirun_engine import EngineNet
from lib.tracing.data_utils import DataLoader
...
PARAMETER_DEFAULTS = {
    ...
    'ph': [0, 0.1, 0.2],
    ## If hospitalized, daily rate entering in R based on age category
    'lamdahr': [0, .083, .033],
    ## If hospitalized, daily rate entering in D based on age category
    'lamdahd': [0, .0031, .0155],
...
def main(args):
    ...
    if args.agent:
        ...
        args.is_learning_agent = any((agent_type.__contains__(typ) for typ in ('sl', 'rl')))
        # initialize args.shared if it has not been supplied from outside and we need it (i.e. when using a RecordAgent)
...
#             stats.sim_summary.update(enumerate(all_events))
    ...
    if args.summary_print is not None:
        return stats.full_summary(args.summary_splits, args.summary_print, args.r_window), extra_return
